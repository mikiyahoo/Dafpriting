  ...
  return (
    <>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "bg-bgPure/95 backdrop-blur-md shadow-sm" : "bg-transparent"
      }`}>
          ...
          <div className="flex items-center justify-between h-20">
            <Link href="/" className="flex items-center gap-2">
              <Image 
                ...
                priority
              />
              <span className={`text-lg font-black transition-colors ${scrolled ? "text-textMain" : "text-textMain"}`}>Daf Printing</span>
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`text-sm font-medium tracking-wide uppercase transition-colors ${scrolled ? "text-textMuted hover:text-primary" : "text-textMain/80 hover:text-textMain"}`}
                >
                  {link.label}
                </Link>
              ))}
              <button
                ...
                className={`inline-flex items-center gap-2 px-6 py-3 bg-primary text-bgPure text-sm font-semibold tracking-wide rounded-full hover:bg-primary/90 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer`}
              >
                Request Quote
                <ArrowRight size={16} />
              </button>
              ...
              <div className="p-6">
                ...
                <div className="mt-8">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full px-6 py-4 bg-primary text-bgPure text-base font-bold tracking-wide rounded-xl hover:bg-primary/90 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer"
                  >
